...
function switchBus($scope, $http) {
	...
	$scope.drawMap = function () {
		...
		$http.get('/media/stations/' + $scope.busNumber + '_stations.json').success(function (data) {
			markers = data;
			initialize(markers, _pathsID);
		});
	};
	$scope.drawMap();
...
function initialize(markers, _pathsID) {
	...
	for (var i = 0; i < markers.length; i++) {
		if (markers[i].Coords) {
			...
			var _path = {"lat" : markers[i].Coords[0], "lng" : markers[i].Coords[1]};
